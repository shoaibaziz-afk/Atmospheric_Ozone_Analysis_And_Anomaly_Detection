...
def train_multi_factor_model(df):
    ...
    df['predicted_ozone_multi'] = multi_model.predict(X_multi)
    df['residual_multi'] = df['Ozone_ppbv'] - df['predicted_ozone_multi']
    
    # Detect anomalies using IQR method
    Q1_multi = df['residual_multi'].quantile(0.25)
    Q3_multi = df['residual_multi'].quantile(0.75)
    IQR_multi = Q3_multi - Q1_multi

    lower_bound_multi = Q1_multi - 1.5 * IQR_multi
    upper_bound_multi = Q3_multi + 1.5 * IQR_multi

    df['is_anomaly_multi'] = (df['residual_multi'] < lower_bound_multi) | (df['residual_multi'] > upper_bound_multi)
    
    print("Anomalies found by multi-factor model:", df['is_anomaly_multi'].sum())
    
    return df, multi_model

def plot_geographic_anomalies(df):
    """Plot geographic distribution of anomalies"""
    plt.figure(figsize=(10, 6))
    plt.scatter(df['Longitude'], df['Latitude'], alpha=0.1, s=2, color='grey', label='Normal')
    plt.scatter(df[df['is_anomaly_multi']]['Longitude'],
                df[df['is_anomaly_multi']]['Latitude'],
...
def plot_anomaly_rate_heatmap(df):
    ...
    grouped = df.groupby([pd.cut(df['Latitude'], lat_bins), 
                           pd.cut(df['Longitude'], lon_bins)])
...
def plot_monthly_anomaly_rate(df):
    """Plot anomaly rate by month"""
    monthly_anomaly_rate = df.groupby('Month')['is_anomaly_multi'].mean() * 100
...
def show_top_anomalies(df, n=10):
    """Display top anomalies"""
    top_anomalies = df.nlargest(n, 'residual_multi')
...
def plot_latitude_month_heatmap(df):
    """Create heatmap of anomaly rate by latitude and month"""
    pivot_table = df.groupby(['Latitude', 'Month'])['is_anomaly_multi'].mean().unstack()

    plt.figure(figsize=(14, 8))
    im = plt.imshow(pivot_table.values, 
                    cmap='RdBu_r',
                    aspect='auto', 
                    extent=[1, 12, df['Latitude'].max(), df['Latitude'].min()])
...
def plot_monthly_facet_grid(df):
    ...
    for i, month in enumerate(range(1, 13)):
        ...
        hexbin = axes[i].hexbin(month_df['Longitude'], month_df['Latitude'], 
                                C=month_df['is_anomaly_multi'], 
...
def plot_3d_anomaly_distribution(df):
